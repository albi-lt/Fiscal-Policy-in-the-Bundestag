...
Ypatch=[irf_estimates{ii}(1,:) fliplr(irf_estimates{ii}(5,:))];
Ypatch2=[irf_estimates{ii}(2,:) fliplr(irf_estimates{ii}(4,:))];


IRFpatch=patch(Xpatch,Ypatch,[0.7 0.78 1]);
IRFpatch2=patch(Xpatch,Ypatch2,[0.7 0.78 1]);

set(IRFpatch,'facealpha',0.5);
set(IRFpatch2,'facealpha',0.25);


set(IRFpatch,'edgecolor','none');
set(IRFpatch2,'edgecolor','none');


plot(irf_estimates{ii}(3,:),'Color',[0.4 0.4 1],'LineWidth',2);
plot([1,IRFperiods],[0 0],'k--');
hold off
minband=min(irf_estimates{ii}(1,:));
maxband=max(irf_estimates{ii}(5,:));
