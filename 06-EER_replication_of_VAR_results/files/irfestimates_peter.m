...
if favar_FAVAR==1
    ...
    for ii=1:n
        ...
        for jj=1:n
            ...
            for kk=1:IRFperiods
                % compute first the lower bound
                irf_estimates{ii,jj}(1,kk)=quantile(irf_record{ii,jj}(:,kk),0.05);
                irf_estimates{ii,jj}(2,kk)=quantile(irf_record{ii,jj}(:,kk),0.16);
irf_estimates{ii,jj}(3,kk)=quantile(irf_record{ii,jj}(:,kk),0.5);
irf_estimates{ii,jj}(4,kk)=quantile(irf_record{ii,jj}(:,kk),0.84);
irf_estimates{ii,jj}(5,kk)=quantile(irf_record{ii,jj}(:,kk),0.95);
