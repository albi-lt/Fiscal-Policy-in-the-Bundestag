...
data = pd.read_pickle('../../data/df_bundestag_speeches_1960_preprocessed.pkl') #rename before pushing
...
id2word.filter_extremes(no_below = 20, no_above=1.0, keep_n=None) #all words that appear less than 20 times in the corpus are removed from the vocabulary
...
id2word.save('id2word-text_preprocessed.dict')  # save dict to disk
corpora.MmCorpus.serialize('corpus-text_preprocessed.mm', corpus)  # save corpus to disk
...
#limit documents as of 1970
df_topicweights_1970 = df_topicweights.loc[170165:] #as of index 170165
...
#load sentiment obtained from doc2vec model

df_sentiment = pd.read_csv('\data\sentiment_scores.csv',index_col = 'Unnamed: 0')
...
sum_df_endo_topic_weights = pd.Series(df_endo_topic_weights.sum(axis=0)) #endogenous topic weights for each doc
...
df_sentiment_1970_grouped.to_csv('data/quarterly_sentiment_decomposed.csv') #quarterly sentiment scores and sentiment exogenous and sentiment endogenous